...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python pred_err_vary_n_nl_linear.py <n>", flush=True)
        sys.exit(1)
    ...
    try:
        ...
        df_recon.to_csv(f"Results/pred_nl_comp_lin/recon_result_{n}.csv", index=False)
        ...
        df_pred = pd.DataFrame([{
            ...
            "pred_clustered_linear": result["pred_clustered_linear"],
            "pred_same_cluster_linear": result["pred_same_cluster_linear"],
            "pred_distinct_clusters_linear": result["pred_distinct_clusters_linear"]
        }])
        df_pred.to_csv(f"Results/pred_nl_comp_lin/predict_result_{n}.csv", index=False)
        ...
        for metric in ["clustered", "same_cluster", "distinct_clusters"]:
            ...
            plt.tight_layout()
            plt.savefig(f"Plots/pred_nl_comp_lin/{metric}_n_{n}.png")
            plt.close()
