...
# New function for cluster non-linear denoisers using a single neural network (for single-cluster setup)
def make_cluster_non_linear_denoisers_single(eb, f_theta):
    """
    Returns a dictionary of non-linear denoising functions for a single-cluster setup using a shared neural network.

    Parameters
    ----------
    eb : Empirical Bayes model (e.g., NonparEB)
    f_theta : callable
        A neural network function mapping support points to scalar outputs.

    Returns
    -------
    dict
        Dictionary with a single key (cluster index 0) mapping to a lambda that computes E[f_theta(U) | f].
    """
    return {
        0: lambda f, mu, cov: eb.non_linear_denoiser(f, mu, cov, f_theta)
    }


# New function for cluster non-linear denoisers using neural networks
def make_cluster_non_linear_denoisers(eb, f_theta_list):
    """
    Returns a dictionary of non-linear denoising functions, one per cluster, using provided neural networks.

    Parameters
    ----------
    eb : Empirical Bayes model (e.g., NonparEB)
    f_theta_list : list of callables
        Each f_theta is a neural network function mapping support points to scalar outputs.

    Returns
    -------
    dict
        Dictionary mapping cluster index to a lambda that computes E[f_theta(U) | f].
    """
    return {
        cluster_idx: (lambda f, mu, cov, f_theta=f_theta_list[cluster_idx]: eb.non_linear_denoiser(f, mu, cov, f_theta))
        for cluster_idx in range(len(f_theta_list))
    }
...
class NonparEB(_BaseEmpiricalBayes):
    ...
    def non_linear_denoiser(self, f, mu, cov, f_theta, eps=1e-8):
        """
        Compute the posterior expectation E[f_theta(U) | f] where f = MU + SZ.
        ...
        cov : ndarray (d, d)
            Covariance matrix.
        f_theta : callable
            A neural network function taking ndarray of shape (nsupp, d) and returning (nsupp,).
        eps : float
        ...
        -------
        output : ndarray (n,)
            Estimated posterior expectations E[f_theta(U) | f_i] for i in 1,...,n.
        """
        covInv = np.linalg.pinv(cov)  # Use pseudo-inverse for numerical stability
        P = _get_P(f, self.Z, mu, covInv, self.pi, eps=eps)  # shape (n, nsupp)

        f_Z = f_theta(self.Z)  # shape (nsupp,)
        return P @ f_Z  # shape (n,)
